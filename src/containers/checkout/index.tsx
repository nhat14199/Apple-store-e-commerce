...
export default function Checkout() {
  const { total }: any = useParams();
  const cost = +total;
  const nummber = cost * 0.1;

  const getLocalStorage: any = localStorage.getItem("user");
  const info = JSON.parse(getLocalStorage);
  const notify = () => toast("Order Success , Please check your email ");

  return (
    <>
      <section className="bg-gray-200  antialiased dark:bg-gray-200 py-4 pt-20 flex justify-center ">
        <form
          action="#"
          className=" max-w-screen-xl px-2 2xl:px-0 bg-slate-50 p-6 rounded-lg shadow-xl "
        >
          ...
          <div className="mt-6 sm:mt-8 lg:flex lg:items-start lg:gap-12 xl:gap-16">
            ...
            <div className="mt-6 w-full space-y-6 sm:mt-8 lg:mt-0 lg:max-w-xs xl:max-w-md">
              ...
              {info == null ? (
                ...
                <button
                  className="space-y-3 bg-blue-600 hover:bg-blue-700 text-white p-2  mt-1 w-full "
                  onClick={() => notify()}
                >
                  Buy Now
                </button>
